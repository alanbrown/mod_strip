...
var box = function(spec, my) {
  ...
  socket_submit = function(data) {
    if(my.mode === my.MODE_NORMAL) {
      ...
      else if(wikipedia_r.test(data.input)) {
        submit_url = 'http://wikipedia.org/wiki/' +
            escape(data.input.split(':')[1]);
      }
      else {
